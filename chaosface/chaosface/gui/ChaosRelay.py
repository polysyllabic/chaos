...
class ChaosRelay:
  ...
  def __init__(self):
    ...
    self.ui_streamer_text_scale = 2.0
  ...
  def reset_all(self):
    ...
    self.set_ui_streamer_text_scale(self.get_attribute('ui_streamer_text_scale'))
  ...
  def set_vote_delay(self, value):
    delay = max(0.0, float(value))
    with self._lock:
      self.chaos_config['vote_delay'] = delay
  ...
  def set_overlay_current_votes_text_side(self, value):
  ...
  def set_overlay_active_mods_text_side(self, value):
    side = str(value or '').strip().lower()
    if side not in ('left', 'right'):
      side = 'right'
  ...
  def set_ui_streamer_text_scale(self, value):
    try:
      scale = float(value)
    except (TypeError, ValueError):
      scale = 2.0
    scale = max(0.5, min(4.0, scale))
    self._set_value('ui_streamer_text_scale', scale, 'ui_streamer_text_scale')
  ...
  def set_overlay_http_port(self, value):
    ...
    if port > 65535:
      port = 65535
    self._set_value('overlay_http_port', port, 'overlay_http_port')
