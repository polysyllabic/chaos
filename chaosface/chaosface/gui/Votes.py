  ...
  def init(self, model):
    ...
    with flx.VBox(flex=0):
      with flx.HFix(flex=1):
        self.voteLabel = flx.Label(flex=0,style=model.get_value('title_text_style'), text="Total Votes: " + str(int(totalVotes)) )
        self.blankLabel = flx.Label(flex=0,style=model.get_value('title_text_style'), text=" ")
        
      with flx.HFix(flex=1):
        with flx.VFix(flex=1):
          for i in range(model.get_value('vote_options')):
            self.progress.append(flx.ProgressBar(flex=2, value=1.0/len(model.relay.votes), text='{percent}', style=model.get_value('progress_style')))
              
        with flx.VFix(flex=1):
          for i in range(model.get_value('vote_options')):
            self.label.append(flx.Label(flex=1,style=model.get_value('mod_text_style'), text=str(i+1)))
