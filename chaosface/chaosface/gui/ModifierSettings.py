...
class ModifierSettings(flx.PyWidget):
  def init(self, relay):
    self.relay = relay
    self.settingsView = ModifierSettingsView(self)
    

class ModifierSettingsView(flx.PyWidget):
  ...
  def init(self, model):
    ...
    with flx.VSplit(flex=1):
      flx.Label(style="text-align:center", text="Modifiers" )
      with flx.VBox(flex=1):
        flx.Widget(flex=1)
        self.gameLabel = flx.Label(style="font-weight:bold;font-size:large", text=str(self.model.relay.gameName))
        self.gameErrors = flx.Label(text=f"Loaded with {str(self.model.relay.gameErrors)} errors")
        flx.Widget(flex=1)
      with flx.HBox():
        with flx.VBox():
        ...
        with flx.VBox(flex=1):
          ...
          self.announceMods = flx.CheckBox(flex=0, text="Announce Mods in Chat", value=self.model.relay.announce_mods)
      with flx.HBox():
        flx.Widget(flex=1)
