...
class SettingsView(flx.PyWidget):
  def init(self, model):
    ...
    with flx.VSplit(flex=1):
      ...
      with flx.HBox():
        ...
        with flx.VBox(flex=1):
          self.announceMods = flx.CheckBox(text="Announce Mods in Chat", checked=self.model.relay.announceMods)
      with flx.HBox():
        flx.Widget(flex=1)
  ...
  def _save_button_clicked(self, *events):
    ...
    self.model.relay.set_announceMods(self.announceMods.checked)
