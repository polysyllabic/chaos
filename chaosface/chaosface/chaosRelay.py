...
#  allMods = flx.ListProp(["1", "2", "3", "4", "5", "6"], settable=True)
  ...
  # Engine Settings
  piHost = flx.StringProp(get_attribute(chaosConfig, "pi_host", "192.168.1.232"), settable=True)
  talkPort = flx.IntProp(get_attribute(chaosConfig, "talk_port", 5555), settable=True)
  listenPort = flx.IntProp(get_attribute(chaosConfig, "listen_port", 5556), settable=True)

  announceMods = flx.BoolProp(get_attribute(chaosConfig, "announce_mods", False), settable=True)
...
#      self.chaosConfig = json.load(json_data_file)
  ...
  def openConfig(self, configFile):
    ...
    self.set_bot_oauth(get_attribute(self.chaosConfig, "bot_oauth", "oauth:s70n7o8wyfh7x7hp6vl16wxwr8ng7s"))
    self.set_channel_name(get_attribute(self.chaosConfig, "channel_name", "polysyl"))
    
    self.set_piHost(get_attribute(self.chaosConfig, "pi_host", "192.168.1.232"))
    self.set_talkPort(get_attribute(self.chaosConfig, "talk_port", 5555))
    self.set_listenPort(get_attribute(self.chaosConfig, "listen_port", 5556))
